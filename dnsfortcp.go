	...
	for ; ; n++ {
		...
		case 0x00:
			...
			if size != int(labelsize) {
				log.Printf("Erro na leitura do %d, bytes de %x", size, label)
			}
		...
		case 0xC0:
			...
			if ptr > 10 {
				return "Muitos ponteiros comprimidos", offset
			}
			ptr++
...
func parseDNSMsg(data []byte) dnsMsg {
	...
	if msg.answer_num > 0 {
		log.Printf("numero da resposta: %d cursor: %d", msg.answer_num, cursor)
	...
	if msg.authority_num > 0 {
		log.Printf("numero da autoridade: %d cursor: %d", msg.authority_num, cursor)
	...
	if msg.additional_num > 0 {
		log.Printf("numero adicional: %d cursor: %d", msg.additional_num, cursor)
